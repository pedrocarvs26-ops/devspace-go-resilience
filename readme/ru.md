...
Dev Space Go — это самостоятельно размещаемый MCP-сервер, который позволяет ИИ-ассистентам читать, редактировать, искать и запускать код в ваших реальных локальных проектах — ваши файлы, ваши инструменты, ваш терминал — без загрузки чего-либо третьим лицам. Вы запускаете его на своём компьютере.
...
Для веб-версии ChatGPT (требуется HTTPS) Dev Space Go автоматически запускает туннель:
