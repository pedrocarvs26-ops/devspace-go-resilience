...
Dev Space Go 是一个自托管的 MCP 服务器，让 AI 助手能够在您真实的本地项目中读取、编辑、搜索和运行代码——您的文件、您的工具、您的终端——无需上传任何内容给第三方。您在机器上运行它，通过您控制的隧道公开它.
...
对于 ChatGPT 网页版（需要 HTTPS），Dev Space Go 会自动启动隧道：
